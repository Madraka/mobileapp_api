...
class ApiController extends AppController
{
    ...
    public function pastPromotions(){
      ...
      //  $this->loadModel('WebsiteTap');
        $this->loadModel('ProfileVisit');
        $this->loadModel('Follower');
        $this->loadModel("PromotionVideoVote");
        ...
        if(count($user_details)> 0 ){
            ...
            $active_promotions = $this->Promotion->getUserActivePromotions($user_id,$created);

            if(count($active_promotions) > 0){

                foreach($active_promotions as $key=>$val){

                   // $active_website_tap_count =  $this->WebsiteTap->getCount($user_id,$val['Promotion']['id']);
                    $active_profile_visit_count = $this->ProfileVisit->getProfileVisitCount($user_id,$val['Promotion']['id']);

                    $active_follower_count = $this->Follower->getUserFollowersAgainstPromotionID($user_id,$val['Promotion']['id']);
                    $active_up_vote_count = $this->PromotionVideoVote->getVoteCount($user_id,$val['Promotion']['id'],1);

                   // $active_promotions[$key]['Promotion']['website_tap_count'] = $active_website_tap_count;

                    $active_promotions[$key]['Promotion']['profile_visit_count'] = $active_profile_visit_count;
                    $active_promotions[$key]['Promotion']['follower_count'] = $active_follower_count;
            ...
            $completed_promotions = $this->Promotion->getUserCompletedPromotions($user_id,$created);


            if(count($completed_promotions) > 0){

                foreach($completed_promotions as $key=>$val){



                   // $completed_website_tap_count =  $this->WebsiteTap->getCount($user_id,$val['Promotion']['id']);
                    $completed_profile_visit_count = $this->ProfileVisit->getProfileVisitCount($user_id,$val['Promotion']['id']);

                    $completed_follower_count = $this->Follower->getUserFollowersAgainstPromotionID($user_id,$val['Promotion']['id']);


                    //$completed_promotions[$key]['Promotion']['completed_website_tap_count'] = $completed_website_tap_count;
                    $completed_promotions[$key]['Promotion']['completed_profile_visit_count'] = $completed_profile_visit_count;
                    $completed_promotions[$key]['Promotion']['completed_follower_count'] = $completed_follower_count;
                         }

            }


            $non_active_promotions = $this->Promotion->getNonActivePromotions($user_id);

            if(count($non_active_promotions) > 0){

                foreach($non_active_promotions as $key=>$val){



                    //$non_active_website_tap_count =  $this->WebsiteTap->getCount($user_id,$val['Promotion']['id']);
                    $non_active__profile_visit_count = $this->ProfileVisit->getProfileVisitCount($user_id,$val['Promotion']['id']);

                    $non_active__follower_count = $this->Follower->getUserFollowersAgainstPromotionID($user_id,$val['Promotion']['id']);


                  //  $non_active_promotions[$key]['Promotion']['non_active_website_tap_count'] = $non_active_website_tap_count;
                    $non_active_promotions[$key]['Promotion']['non_active_profile_visit_count'] = $non_active__profile_visit_count;
                    $non_active_promotions[$key]['Promotion']['non_active_follower_count'] = $non_active__follower_count;
                      }

            }

            $array['active'] = $active_promotions;
            $array['completed'] = $completed_promotions;
            $array['non_active'] = $non_active_promotions;
        ...
        $this->set("past_promotions",$array);
        ...
        if ($this->request->isPost()) {
            ...
            if(isset($data['user_id'])){
                ...
                if(count($video_details) > 0){
                   ...
                   $views =  $video_details['Video']['view'];
                   $video_user_id =  $video_details['Video']['user_id'];

                   if($video_user_id == $data['user_id']){
    ...
    public function showUserDetail()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if(isset($data['other_user_id']) || $other_user_id > 0){
...
//pr($comments);
    ...
    public function showFollowingVideos(){
        ...
        if ($this->request->isPost()) {
            ...
            $follower_details = $this->Follower->getUserFollowing($user_id);
            ...
            foreach($follower_details as $key=>$va){

               $ids[$key] = $va['FollowingList']['id'];
            }
    ...
    public function addUserImage()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (isset($data['profile_pic'])) {
               ...
               // $resize_image = Utility::resize_image($filePath, 220, 220, UPLOADS_FOLDER_URI . "/images/".uniqid().time().$user_id . '.'."png");

                //unlink($filePath);

                //$user['profile_pic'] = $resize_image;
                $user['profile_pic'] = $filePath;
